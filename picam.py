...
if __name__ == "__main__":
   print("\n# This program calculates distance and angle for a color object.")

   select_hsv="y" # カラーオブジェクトをマウスで選ぶときは "y" 
   ...
   now=time.time()
   start=now
   print("\n # Input 'q' to stop in View window")
   print("# time   dist(m) angle(rad)")
   key=cv2.waitKey(1) # 1msの間キー入力をまつ
   while key!=ord("q"): # アスキーコードで比較
      dis, rad, frame = picam.calc_dist_theta(lower_light, upper_light)
      if dis==None:
         print("\r %5.2f  None  None" % (now-start),end="")
      else:
         print("\r %5.2f %7.2f %7.2f" % (now-start,dis,rad),end="")
